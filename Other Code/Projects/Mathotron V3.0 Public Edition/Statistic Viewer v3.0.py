...
while 1 == 1:
 ...
 if data == "1":
   messagebox.showinfo("Mathotron Statistic Viewer","would you like to view how many times mathotron has been used to times, divide, add ,subtract or has calculated times tables for specific numbers")
   type = input("1 for times,2 for divide,3 for add, 4 for subtract,5 for times tables")
 if type == "1":
      data = open('User_Data.txt').read()
...
if type == "5":
      data = open('User_Data.txt').read()
      count = data.count("times tables")
      messagebox.showinfo("Mathotron Statistic Viewer","Reading Data")
      with open('User_Data.txt') as f:
          for line in f:
              count += line.count("times tables")
      messagebox.showinfo("Mathotron Statistic Viewer","Mathotron Has Calculated Times Tables For Specific Numbers " + str(count) + " Times")
